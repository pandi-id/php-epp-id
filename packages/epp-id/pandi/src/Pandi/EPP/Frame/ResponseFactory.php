...
namespace Pandi\EPP\Frame;

use Pandi\EPP\ObjectSpec;
use DOMDocument;
use DOMXPath;
...
class ResponseFactory
{
    public static function build($buffer)
    {
        ...
        $xml->registerNodeClass('\DOMElement', '\Pandi\EPP\DOM\DOMElement');
        $xml->loadXML($buffer);
        ...
        foreach ($nodes->item(0)->childNodes as $node) {
            ...
            if ($frame_type === 'response') {
                ...
                if ($results->length > 0) {
                    return new \Pandi\EPP\Frame\Response\MessageQueue($xml);
                }
